...
def get_valid_name(prompt):
    while True:
        name = input(f"{prompt} (or 0 to cancel): ").strip().lower()
        if name == "0":
            return "0"
        if name != "":
            return name
...
class Inventory:
    ...
    def load_inventory_data(self, username):
        ...
        self.expired_inventory = []
        try:
            with open(f"saves/{self.username}_inventory.txt", "r") as file:  # Load a user's inventory data from a text file inside the saves folder
                while True:
                    line = file.readline()
                    if line == "":
                        break
        ...
        try:
            with open(f"saves/{self.username}_expired_inventory.txt", "r") as file:  # Load a user's expired inventory data from a text file inside the saves folder
                while True:
                    line = file.readline()
                    if line == "":
                        break
    ...
    def save_inventory_data(self):
        with open(f"saves/{self.username}_inventory.txt", "w") as file: # Save a user's inventory data into text files inside the saves folder
            for food in self.inventory:
                file.write(food.to_file_line() + "\n")

        with open(f"saves/{self.username}_expired_inventory.txt", "w") as file: # Save a user's expired inventory data into text files inside the saves folder
            for food in self.expired_inventory:
                file.write(food.to_file_line() + "\n")

    # Reduce remaining expiry days after the user logs in again
    def update_expiry_after_login(self, days_passed):
        ...
        for food in self.inventory:
            food.reduce_expiry_days(days_passed)
    ...
    # Move expired food from current inventory to expired inventory
    ...
    # Display all food items, grouped by food name and sorted by expiry days
    ...
    # Get the unit of an existing food item from inventory
    def get_unit_from_inventory(self, name):
        for food in self.inventory:
            if food.name == name:
                return food.unit
        return None
    ...
    def use_food(self, user):
        ...
        unit = self.get_unit_from_inventory(name)
        if unit is None:
    ...
    # Shared food reduction logic for use_food, remove_food, and cook_recipe
    def reduce_food_from_inventory(self, name, quantity_reduced, action="Used", add_nutrition=True):
        ...
        new_inventory = [] # Rebuild the inventory list and remove food items with zero quantity
        for food in self.inventory:
    ...
    def clear_expired_inventory(self):
        self.expired_inventory = []
        print("🧹 Expired food removed successfully.")
        self.save_inventory_data()


# Recipe and Nutrition Functions
...
# Cook a recipe, reduce food from the inventory, and add nutrition intake
