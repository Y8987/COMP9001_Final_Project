...
class User:
    ...
    # Reset daily nutrition intake when a new day starts
    ...
    # Calculate how many days have passed since the user's last login
    ...
    def load(self):
        try:
            with (open(f"saves/{self.username}_profile.txt", "r") as file): # Load user profile data from a text file inside the saves folder
                for line in file:
                    line = line.strip()
                    if line.startswith("username: "):
                        self.username = line.replace("username: ", "")
    ...
    def save(self):
        with open(f"saves/{self.username}_profile.txt", "w") as file: # Save user profile data into a text file inside the saves folder
